...
describe("Render Search tests", () => {
  ...
  test("Test for presence of serial number filter", () => {
    const component = shallow(<SearchBox />);
    expect(component.find(FormControlLabel).at(0).props().value).toEqual(
      "SerialNumber"
    );
  });
  test("Test for presence of Brand filter", () => {
    const component = shallow(<SearchBox />);
    expect(component.find(FormControlLabel).at(1).props().value).toEqual(
      "Brand"
    );
  });
  test("Test for presence of Model filter", () => {
    const component = shallow(<SearchBox />);
    expect(component.find(FormControlLabel).at(2).props().value).toEqual(
      "Model"
    );
  });
  test("Test for presence of Status filter", () => {
    const component = shallow(<SearchBox />);
    expect(component.find(FormControlLabel).at(3).props().value).toEqual(
      "Status"
    );
  });
  test("Test for presence of DateBought filter", () => {
    const component = shallow(<SearchBox />);
    expect(component.find(FormControlLabel).at(4).props().value).toEqual(
      "DateBought"
    );
  });
});
