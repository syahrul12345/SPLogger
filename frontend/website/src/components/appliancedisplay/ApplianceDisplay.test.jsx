...
describe('Test with 2 appliance objects as props',() => {
  const component = shallow(
     <ApplianceDisplay appliances={mockAppliances}/>
  )
  ...
  mockAppliances.forEach((mockAppliance,idx) => {
    test(`Find card with Brand field as ${mockAppliance['Brand']}`,() => {
      expect(component.find(Card).at(idx).find(CardContent).childAt(0).text()).toEqual("Brand: " + mockAppliance['Brand'])
    })
    test(`Find card with Model field as ${mockAppliance['Model']}`,() => {
      expect(component.find(Card).at(idx).find(CardContent).childAt(1).text()).toEqual("Model: " + mockAppliance['Model'])
    })
    test(`Find card with status as ${mockAppliance['Status']}`,() => {
      expect(component.find(Card).at(idx).find(CardContent).childAt(2).text()).toEqual("Status: " + mockAppliance['Status'])
    })
    test(`Find card with SerialNumber as ${mockAppliance['SerialNumber']}`,() => {
      expect(component.find(Card).at(idx).find(CardContent).childAt(3).text()).toEqual("SerialNumber: " + mockAppliance['SerialNumber'])
    })
  })
})
